...
This table is **not** auto-synced with `bench_runner.py`. It exists so **mAP / recall** come from **one** procedure: **`scripts/eval_coco_predictions.py`** (pycocotools `COCOeval`, bbox IoU).
...
The README auto-table still comes from `bench_runner`; use the unified table above when you need one `COCOeval` definition for mAP/recall.

#!/usr/bin/env python3
"""Run 3 default YOLOv8+BoxMOT configs and aggregate speed + TrackEval metrics."""
...
def main() -> None:
    ...
    parser.add_argument("--tracker-type", required=True, help="boxmot tracker key, e.g. botsort")
    parser.add_argument("--tracker-label", default="", help="Label used in artifact file names")
    ...
    parser.add_argument("--reuse-existing", action="store_true", help="Reuse existing run reports and skip tracking")
    args = parser.parse_args()

    tracker_type = args.tracker_type.strip().lower()
    tracker_label = args.tracker_label.strip().lower() or tracker_type
    tracker_name = f"yolov8_{tracker_label}"
    ...
    for cfg in CONFIGS:
        ...
        if not args.reuse_existing:
            run_cmd(
                [
                    "python3",
                    "scripts/tracking/run_yolov8_boxmot_mot17.py",
                    "--weights",
                    args.weights,
                    "--source",
                    source,
        ...
        report_path = load_latest_json(
            f"{results_dir.relative_to(repo_root)}/{run_name}_*_run_report.json"
        )
    ...
    out_json = results_dir / f"{tracker_name}_mot17_benchmark.json"
    out_md = results_dir / f"{tracker_name}_mot17_benchmark.md"
    payload = {
    ...
    lines = [
        f"# YOLOv8n + {tracker_name} benchmark (MOT17)",
        "",

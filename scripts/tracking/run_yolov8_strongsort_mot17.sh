...
# Wrapper for typical MOT17 sequence tracking run with StrongSORT.
#
# Example:
...
NAME="${NAME:-yolov8_strongsort_${MOT17_SEQ}}"
...
python3 "${REPO_ROOT}/scripts/tracking/run_yolov8_strongsort_mot17.py" \
  --weights "${WEIGHTS}" \
  --source "${MOT17_SOURCE}" \
  --imgsz "${IMGSZ}" \
  --conf "${CONF}" \

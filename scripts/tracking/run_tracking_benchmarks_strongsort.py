#!/usr/bin/env python3
"""Run 3 default YOLOv8+StrongSORT configs and aggregate speed + TrackEval metrics."""
...
def main() -> None:
    ...
    for cfg in CONFIGS:
        ...
        run_name = f"yolov8_strongsort_{args.mot17_seq}_{suffix}"
        source = f"{args.mot17_root}/MOT17/train/{args.mot17_seq}/img1"

        run_cmd(
            [
                "python3",
                "scripts/tracking/run_yolov8_strongsort_mot17.py",
                "--weights",
                args.weights,
                "--source",
                source,
        ...
        run_cmd(
            ...
            env_overrides={
                ...
                "TRACKER_NAME": f"yolov8_strongsort_{suffix}",
            },
        )

        metrics_path = load_latest_json(
            f"{trackeval_dir.relative_to(repo_root)}/yolov8_strongsort_{suffix}_{args.mot17_seq}_metrics.json"
        )
    ...
    out_json = results_dir / "yolov8_strongsort_mot17_benchmark.json"
    out_md = results_dir / "yolov8_strongsort_mot17_benchmark.md"
    payload = {
    ...
    lines = [
        "# YOLOv8n + StrongSORT benchmark (MOT17)",
        "",

...
def main() -> None:
    ...
    for cfg in CONFIGS:
        conf = cfg["conf"]
        iou = cfg["iou"]
        run_name = f"yolov8_strongsort_{args.mot17_seq}_c{str(conf).replace('.', '')}_i{str(iou).replace('.', '')}"
        ...
        metrics_path = load_latest_json(
            f"{trackeval_dir.relative_to(repo_root)}/yolov8_strongsort_{args.mot17_seq}_metrics.json"
        )

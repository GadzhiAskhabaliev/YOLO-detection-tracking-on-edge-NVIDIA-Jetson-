...
def _ap50(run: dict[str, Any] | None) -> float | None:
    if not run:
        return None
    m = run.get("metrics") or {}
    v = m.get("AP50")
...
def _plot_scatter(
    ...
    *,
    ...
    if not series:
        ax.text(0.5, 0.5, "No points with both AP50 and FPS", ha="center", va="center", fontsize=11)
        ax.set_axis_off()
    else:
...
def regenerate(*, manifest_path: Path | None = None, out_md: Path | None = None) -> Path:
    ...
    rows_md.append("| ID | Model | AP50 | FPS | Run |\n")
    rows_md.append("|----|-------|-------|-----|-----|\n")
    ...
    bars_path = FIG_DIR / "group_b_ap50_bars.png"
    _plot_scatter(
        scatter_series,
        scatter_path,
    ...
    _plot_bars(
        ...
        bar_vals,
        bars_path,
        title=f"Group {group}: AP50 by manifest slot (gray = no run)",
    )
